...
- Use filters (All, Pending, Overdue, Completed) to view specific tasks
...
Share your categories with others to collaborate on tasks. Categories are shared through an invitation system that requires approval from the invited user.
...
- If a calendar event is deleted from your iOS calendar, the corresponding task will also be deleted from the app (including future occurrences of recurring events)
- Import history is stored locally and synced to the cloud, so it persists across app reinstalls
...
- Import history is automatically synced between your devices and persists across app reinstalls
...
- Total Tasks: Shows the count of all tasks with due dates
- Completed: Shows how many tasks with due dates have been completed
- Completion Rate: Percentage of completed tasks out of total tasks
- Active Tasks: Shows tasks with due dates that are not completed

**Note:** All statistics include archived tasks in their calculations.
...
- Only tasks with due dates are included in the statistics
- Tasks without due dates are not counted
...
- Use the "Shopping" category name for shopping lists to keep it at the top
...
- Long press any task for quick access to Edit/Archive
